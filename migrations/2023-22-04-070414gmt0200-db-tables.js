...
module.exports.up = async function (next) {
    ...
    const valuesCarts = initialData.carts
        .map(item => `('${item.id}', '${item.user_id}', TO_TIMESTAMP('${item.created_at}', 'YYYY-MM-DD'), TO_TIMESTAMP('${item.updated_at}', 'YYYY-MM-DD'), '${item.status}')`)
        .join(', ')

    const valuesCartItems = initialData.cart_items
        .map(({cart_id, product_id, count}) => `('${cart_id}', '${product_id}', ${count})`)
        .join(', ')
