...
import { UserCart } from "../models/index";
...
export class CartService {
  ...
  async findByUserId(userId: string): Promise<UserCart> {
    try {
      ...
      return {
        id: cartEntity.id,
        items: await this.getProductsByIds(cartEntity.items.map(item => item.product_id))
      };
    } catch (e) {
      throw e;
    }
  ...
  async findOrCreateByUserId(userId: string): Promise<UserCart> {
  ...
  async updateByUserId(userId: string, { items }: UserCart): Promise<UserCart> {
